...
foreach(array_reverse($array_of_posts) as $current) {
    ...
    echo('<p class="timetext">Posted on ' . $current->time . '<br>By ' . $current->poster . '</p><p class="posttext">' . $current->text . '</p>');
    if (isset($current->file_ext)) {
            echo('<img width="32" height="32" src="imguploads/' . $current->title . $current->file_ext . '" /></div>');
        }
    echo("</div>");
}
?>
